    ...
    describe(adapter + ' adapter', function () {
      ...
      describe('Suite with only one passing test', function () {
        it('should be emitted on suiteStart event', function () {
          expect(collectedData[1][0]).to.be.equal('suiteStart')
        })
        ...
        it('should have no other child suites', function () {
          expect(collectedData[1][1].childSuites).to.have
            .lengthOf(refData[1][1].childSuites.length)
        })

        it('should contain only one test', function () {
          expect(collectedData[1][1].tests).to.have
            .lengthOf(refData[1][1].tests.length)
        })

        describe('Test on start', function () {
          it('should have been emitted on testStart event', function () {
            expect(collectedData[2][0]).to.be.equal('testStart')
          })

          it('should have a name', function () {
            expect(collectedData[2][1].testName).to.be
              .equal(refData[2][1].testName)
          })

          it('should contain the name of the suite parent', function () {
            expect(collectedData[2][1].suiteName).to.be
              .equal(refData[2][1].suiteName)
          })

          it('should have only 2 props: testName and suiteName', function () {
            expect(collectedData[2][1]).to.have.all
        ...
        describe('Test on end', function () {
          it('should have been emitted on testEnd event', function () {
            expect(collectedData[3][0]).to.be.equal('testEnd')
          })

          it('should have the same name as on start', function () {
            expect(collectedData[3][1].testName).to.be
              .equal(refData[3][1].testName)
          })
          ...
          it('should contain status as passed', function () {
            expect(collectedData[3][1].status).to.be
              .equal(refData[3][1].status)
          })

          it('should contain its runtime', function () {
            expect(collectedData[3][1].runtime).to.be
              .closeTo(refData[3][1].runtime, 1)
          })

          it('should have no errors', function () {
            expect(collectedData[3][1].errors).to.be.deep
              .equal(refData[3][1].errors)
          })
        })
      })
    })
