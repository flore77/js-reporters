...
export default class MochaAdapter extends EventEmitter {
  ...
  onSuite (mochaSuite) {
    if (!mochaSuite.root) {
      this.emit('suiteStart', this.idDataMap[mochaSuite.title])
    }
  }

  onTest (mochaTest) {
    this.errors = []
    this.emit('testStart', this.idDataMap[mochaTest.title])
  }
  ...
  onTestEnd (mochaTest) {
    var test = this.idDataMap[mochaTest.title]
  ...
  onSuiteEnd (mochaSuite) {
    if (!mochaSuite.root) {
      this.emit('suiteEnd', this.idDataMap[mochaSuite.title])
    }
  }
